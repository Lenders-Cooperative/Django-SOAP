...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-soap runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-soap.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_soap")
...
def lint(c):
    ...
    """
    c.run("flake8 django-soap tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_soap.__version__))
    c.run("git push --tags")
